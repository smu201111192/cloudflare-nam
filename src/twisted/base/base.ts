...
export class BaseApi<Region extends string> {
  ...
  private isRateLimitError(e: any) {
    ...
    return (
      e.status === TOO_MANY_REQUESTS || e.response?.status === TOO_MANY_REQUESTS
    );
  }
  ...
  private internalRequest<T>(url: string, options: ky.Options): Promise<T> {
    return RequestBase.request<T>(url, options);
  }
  ...
  private async retryRateLimit<T>(
    ...
    const isRateLimitError =
      this.isRateLimitError(e) || this.isServiceUnavailableError(e);
    if (
      !this.rateLimitRetry ||
      !isRateLimitError ||
    ...
    for (let i = 0; i < this.rateLimitRetryAttempts; i++) {
      try {
        ...
        const msToWait =
          (RetryAfter || 0) * 1000 + waitSeconds * 1000 * Math.random();
        // Log
  ...
  protected async request<T>(
    ...
    if (this.debug.logUrls) {
      Logger.uri(url, options, endpoint);
    }
    try {
      const apiResponse = await this.internalRequest(url, options);

      // const { data, headers } = apiResponse
      return {
        // rateLimits: this.getRateLimits(apiResponse.headers),
        response: apiResponse as unknown as T,
      };
    } catch (e) {
      if (forceError) {
        throw e;
